...
@IBDesignable open class RangeMovieSlider: UIControl {
  ...
  @IBInspectable open var trackHighlightTintColor: UIColor = UIColor(red: 0.0, green: 0.45, blue: 0.94, alpha: 1.0) {
    ...
    @IBInspectable open var trackHightInsetLine: Bool = false {
        didSet {
            track.setNeedsDisplay()
        }
    }
  ...
  let track = RangeMovieSliderTrack()
  let lowerKnob = RangeMovieSliderKnob()
  let upperKnob = RangeMovieSliderKnob()
  ...
  // MARK: Touch Tracking
    ...
    private func highlightKnob(_ knob: RangeMovieSliderKnob, knobPosition: Knob) {
 ...
 */
  ...
  func animateKnob(knob: RangeMovieSliderKnob, selected:Bool) {
    CATransaction.begin()
  ...
  func positionForValue(_ value: Double) -> CGPoint {
    ...
    let knobDeltaX: CGFloat = (KnobSize / 2) - RangeMovieSliderKnob.KnobDelta
    let knobDeltaWidth:CGFloat = -(KnobSize - (RangeMovieSliderKnob.KnobDelta * 2))
